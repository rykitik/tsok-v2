...
function initiateDrag(e) {
  let obj = e.target.classList.contains('drag')
    ? e.target
    : e.target.parentNode.classList.contains('drag')
    ? e.target.parentNode
    : null;
  ...
  if (dragElement.object) {
    ...
    for (let drop of drops) {
      let horizontalOverlap =
        Math.abs(
          drag.offsetLeft + drag.offsetWidth / 2 -
            (drop.offsetLeft + drop.offsetWidth / 2)
        ) < (drag.offsetWidth + drop.offsetWidth) / 2;
      let verticalOverlap =
        Math.abs(
          drag.offsetTop + drag.offsetHeight / 2 -
            (drop.offsetTop + drop.offsetHeight / 2)
        ) < (drag.offsetHeight + drop.offsetHeight) / 2;
